package yfrp.skindata;

public class SkinData {
    public static final SkinModel WIDE = new SkinModel(
            new ModelCube(-4, 24, -4, 8, 8, 8, 0, 0),
            new ModelCube(-4, 12, -2, 8, 12, 4, 16, 16),
            new ModelCube(4, 12, -2, 3, 12, 4, 40, 16),
            new ModelCube(-4, 12, -2, -3, 12, 4, 32, 48),
            new ModelCube(0, 0, -2, 4, 12, 4, 0, 16),
            new ModelCube(0, 0, -2, -4, 12, 4, 16, 48));
    public static final SkinModel SLIM = new SkinModel(
            new ModelCube(-4, 24, -4, 8, 8, 8, 0, 0),
            new ModelCube(-4, 12, -2, 8, 12, 4, 16, 16),
            new ModelCube(4, 12, -2, 4, 12, 4, 40, 16),
            new ModelCube(-4, 12, -2, -4, 12, 4, 32, 48),
            new ModelCube(0, 0, -2, 4, 12, 4, 0, 16),
            new ModelCube(0, 0, -2, -4, 12, 4, 16, 48));
}
